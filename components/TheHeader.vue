  ...
  <header class="row justify-between align-center">
    ...
    <div class="col d-flex justify-end">
      ...
      <img
        ...
        class="user_avatar"
        src="~/assets/images/nuxt-white.svg"
      />
      <div class="user_info" v-if="infoVisible">
        <div>{{ userStore.user.name }}</div>
        <hr class="my-space-sm" />
        <BaseButton class="btn__primary" @click="logout">Logout</BaseButton>
      </div>
    </div>
  </header>
</template>
...
header {
  ...
  width: 100%;
  z-index: 1;
  background-color: var(--secondary-color-350);
